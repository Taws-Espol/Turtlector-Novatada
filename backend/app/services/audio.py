...
class AudioService:
    ...
    def transcribe_base64_audio(
        ...
        wav_bytes = (
            self._convert_mp3_to_wav(audio_bytes) if audio_format == "mp3" else audio_bytes
        )
    ...
    def _detect_audio_format(self, audio_bytes: bytes, audio_mime_type: str | None) -> str:
        ...
        raise ValueError("Unsupported audio format: only WAV and MP3 are accepted")
    ...
    def _convert_mp3_to_wav(self, mp3_bytes: bytes) -> bytes:
        try:
            ...
            raise RuntimeError("Could not convert MP3 to WAV. Verify ffmpeg is installed.") from exc
